### 一、历史逻辑
#### 自动准备
1. 在自动点击循环中过程中，每5s（可配置）通过OCR识别joinOCRArea区域，如果不为“加入”，则跳出循环点击。并切换按钮状态。
2. 注意，手动点击按钮停止的循环不需要执行该步骤。识别顶部区域levelOcrArea，根据不同的层数选择不同的阵容，可配置，依次点击区域：planButtonLocation、$configLocation、levelOcrArea、levelOcrArea, 每次点击间隔100ms（加上随机数）
   配置如下
3. 点击准备区域prepareButtonLocation
4. 结束

#### 功能1:优化自动加入流程
点击开始后
1. 先点击聊天框工具
2. 点击队伍
3. 点击选择难度
4. 点击空白区域
5. 开始自动点击（原来的自动点击逻辑）

```
#聊天框位置
chatBoxLocation = (990, 1020, 1010, 1040)

队伍位置
teamButtonLocation = (80, 1410, 100, 1430)

选择难度位置
selectDifficultyButtonLocation = (610, 1720, 630, 1740)

空白区域teamWhileBlankArea: { left: 600, top: 500, right: 630, bottom: 530 }


```

#### 功能2:自动召唤词条
1. 点击准备后，每3s识别准备区域，不是“已准备”时，推出循环。休眠8s，随后点击召唤英雄区域10次。
2. 结束

```
#召唤英雄位置
召唤 = (730, 2200, 850, 2300)

```

3. 关闭飘字(70, 730, 90, 750)、点击流畅(70, 570, 90, 590) done

#### 提前检测颜色。减少ocr识别
1. 优化词条OCR识别：识别词条前判断颜色，
    - 在识别OCR前，判断多个点的颜色是否相同，相同则进行ocr识别，否次不进行OCR识别。
    - { x: 410, y: 816 },
      { x: 450, y: 816 },
      { x: 500, y: 816 },
      { x: 550, y: 816 },
      { x: 600, y: 816 },
      { x: 670, y: 816 }
      配置：检验颜色开关、多个点的坐标集合[(400, 816),(450, 816),(620, 816),(680, 816),(540, 816)]。threshold=20
      注意：打印每个点的颜色信息。
      参考比较颜色代码：
```js

function isSimilar(color1, color2, threshold) {
   let r1 = (color1 >> 16) & 0xff;
   let g1 = (color1 >> 8) & 0xff;
   let b1 = color1 & 0xff;
   let r2 = (color2 >> 16) & 0xff;
   let g2 = (color2 >> 8) & 0xff;
   let b2 = color2 & 0xff;
   let diff = Math.abs(r1 - r2) + Math.abs(g1 - g2) + Math.abs(b1 - b2);
   return diff <= threshold * 3; // 若总分差小于阈值*3认为相似
}
```
2. 优化自动点击停止判断OCR识别
    - 判断给定的区域，区域内选取点的8个数，如果8个点颜色存在不同，则停止点击。
      配置：检验颜色开关、多个点的坐标集合[(1010, 600),(1010, 700),(1010, 800),(1010, 900),(1010, 1000)]。threshold=10
      注意：打印每个点的颜色信息。

### 一、新增逻辑

连续合作：
自动选择词条：新增停止时机。词条OCR颜色预检的同时，新增返回按钮区域的颜色点检查。如果检查点的颜色同相同则停止自动选词条，检测点[(680， 2060), (880， 2060), (680， 2100), (880， 2100)]。
此时进入了游戏结算画面（游戏已结束），进行自动点击宝箱操作，boxArea = (270,1150,290,1170), 4s后点击结算页面的返回按钮 settlementBackButtonArea = (680, 2060, 880, 2100)。
同时停止选词条功能，即词条按钮为关闭状态。 
开启自动点击功能，寻找下一把合作（原自动点击逻辑）。
     



### 二、坐标区域配置

```python
# ========== OCR识别区域 ==========
# 加入按钮识别区域（检测游戏是否就绪）
joinOcrArea = (580, 1710, 730, 1770)

# 当前楼层识别区域
levelOcrArea = (420, 280, 650, 360)

# ========== 按钮点击区域 ==========
# 方案主入口按钮
planButtonLocation = (880, 1950, 940, 1990)

# 方案2按钮（用于彩虹5层）
plan2ButtonLocation = (480, 720, 500, 740)

# 方案4按钮（用于彩虹3-4层）
plan4ButtonLocation = (830, 720, 850, 740)

# 准备按钮
prepareButtonLocation = (500, 1930, 600, 2000)
```

### 三、异常处理说明

| 异常场景 | 处理方式        |
|----------|-------------|
| OCR识别“加入”失败 | 等待下次检测      |
| 楼层识别失败/为空 | 使用默认方案，记录日志 |

### 四、日志输出规范

```
[时间] 状态: 检测到游戏未就绪，等待中...
[时间] 状态: 游戏就绪
[时间] 楼层识别: 彩虹4层 → 使用方案4
[时间] 点击: 方案入口 → 方案4按钮 → 准备按钮
```

---


TODO 灼烧岩浆 移动到 核能增幅 前